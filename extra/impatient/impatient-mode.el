...
(defvar imp-default-user-filters
  '((html-mode . nil)
    (web-mode  . nil)
    (js-mode   . imp-javascript-filter))
  "Alist indicating which filter should be used for which modes.")

(defvar imp-mode-type 'write-file-hooks
  "When to triggle the refresh, every type of every save")
...
(define-minor-mode impatient-mode
  ...
  (if (not impatient-mode)
      (progn
        (imp--cleanup-timer)
        (remove-hook imp-mode-type #'imp--on-change t))
    (add-hook 'kill-buffer-hook #'imp--cleanup-timer nil t)
    (add-hook imp-mode-type #'imp--on-change nil t)
    (imp-remove-user-filter)))

(defun imp/switch-type ()
  "Switch the hook when to triggle refresh, eg:

  `after-change-functions' - every keypressed;
  `write-file-hooks'       - when file saved.\n
"
  (interactive)
  (let ((hook (intern (completing-read "The Hook to triggle refresh: " '("after-change-functions" "write-file-hooks")))))
    (if (boundp hook)
        (progn
          (remove-hook imp-mode-type #'imp--on-change t)
          (setq imp-mode-type hook)
          (add-hook imp-mode-type #'imp--on-change nil t))
      (message "No such hook: '%s', use '%s' instead." hook imp-mode-type))))
...
(defun imp-remove-user-filter ()
  ...
  (interactive)
  (let ((lookup (cl-find-if (lambda (mode) (derived-mode-p (car mode))) imp-default-user-filters)))
    (if lookup
...
(defun imp-javascript-filter (buffer)
  "Wrap js code with <script>"
...
(defun imp-serve-buffer-list (proc)
  ...
  (with-httpd-buffer proc "text/html; charset=utf-8"
    ...
    (dolist (buffer (imp--buffer-list))
      (insert (format "<li><a href=\"imp/%s\">%s</a></li>\n"
...
(defun httpd/imp (proc path _query req)
        ...
        (if live-buffer
            (with-temp-buffer
              ...
              (if (imp--should-not-cache-p path)
                  (httpd-send-header proc mime-type 200 :Cache-Control "no-cache")
                (httpd-send-header proc mime-type 200 :Cache-Control "max-age=60, must-revalidate")))
          (httpd-send-file proc full-file-name req))))
     (t (imp-buffer-enabled-p buffer)
        (with-temp-buffer
          (insert-file-contents index)
          (when (re-search-forward "<<<buffer-title>>>" nil t)
            (replace-match buffer-name))
          (when (re-search-forward "<<<buffer-src-nowait>>>" nil t)
            (replace-match (concat "/imp/buffer-nowait/" buffer-name)))
          (when (re-search-forward "<<<buffer-src>>>" nil t)
            (replace-match (concat "/imp/buffer/" buffer-name)))
          (httpd-send-header proc (httpd-get-mime (file-name-extension index)) 200))))))
...
(defun httpd/imp/buffer (proc path query &rest _)
  ...
  (let* ((buffer-name (file-name-nondirectory path))
         ...
         (req-last-id (cadr (assoc "id" query))))
    (if (imp-buffer-enabled-p buffer)
        (with-current-buffer buffer
          (if (and req-last-id (equal (string-to-number req-last-id) imp-last-state))
...
(defun httpd/imp/buffer-nowait (proc path query &rest _)
  (let* ((buffer-name (file-name-nondirectory path))
         (buffer (get-buffer buffer-name)))
    (if (imp-buffer-enabled-p buffer)
        (with-current-buffer buffer
