      ...
      (er--save-excursion
       (ignore-errors
         ...
         (when (and (region-active-p)
           ...
           (setq best-end (mark))
           (setq best-expand (car try-list))
           (when (and er--show-expansion-message (not (minibufferp)))
             (message "%S" (car try-list))))))
      (setq try-list (cdr try-list)))

    ;; imfine / add `best-expand' to record the chosen. 2021-05-25
    (when best-expand
      (let ((inhibit-message t))
        (message "[expand-region]: %S" best-expand)))
...
(defun er-prepare-for-more-expansions ()
  ...
  (let* ((repeat-key (event-basic-type last-input-event))
    ...
    ;; imfine / add below line to enable `expand-region' with origin keybinds. 2021-05-25
    (push (cons (concat "C-" repeat-key-str) '(er-expand-region 1)) bindings)
    (when repeat-key
