...
(cl-defun httpd-start
    (&key (root "~/vvv") (host "0.0.0.0") (port httpd-port)
          (cache t) (body-decorator (lambda (_ body) body)))
          ...
          (dolist (file (directory-files path))
            (unless (eq ?. (aref file 0))
              (let* ((full (expand-file-name file path))
                     ...
                     (f (encode-coding-string (url-insert-entities-in-string file) 'utf-8)))
