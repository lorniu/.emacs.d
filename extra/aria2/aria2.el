...
(defcustom aria2-list-buffer-name "*aria2*"
  "Name of buffer to use when showing downloads list."
  :type '(string :tag "Buffer name")
  :group 'aria2)

(defcustom aria2-rpc-url "http://localhost:6800/jsonrpc"
  "Url of aria2 daemon server."
  :type 'string
  :group 'aria2)

(defcustom aria2-rpc-token nil
  "Token used by aria2 daemon server."
  :type 'string
  :group 'aria2)

(defcustom aria2-session-file (locate-user-emacs-file "aria2.session")
  "Name of session file.  Will be used with \"--save-session\" and \"--input-file\" options."
  :type 'file
  :group 'aria2)

(defcustom aria2-download-directory (or (getenv "XDG_DOWNLOAD_DIR") (expand-file-name "~/Downloads"))
...
(defconst aria2--cc-file (locate-user-emacs-file "aria2-controller.eieio")
...
(defsubst aria2--port ()
  "Parse url and extra the port."
  (save-match-data
    (if (string-match ":\\([0-9]+\\)/" aria2-rpc-url)
        (string-to-number (match-string 1 aria2-rpc-url))
      6800)))
...
(defun aria2--is-aria-process-p (pid)
  ...
  (let ((proc-attr (process-attributes pid)))
    (and
     (string= "aria2c" (if-let ((p (alist-get 'comm proc-attr))) (file-name-sans-extension p)))
...
(defclass aria2-controller (eieio-persistent)
  ((request-id :initarg :request-id
   ...
   (rpc-url :initarg :rpc-url
            :initform (identity aria2-rpc-url)
            :type string
    ...
    (let ((options (delq nil
                         (append
                          ...
                          `("-D" ;; Start in daemon mode (won't be managed by Emacs).
                            "--enable-rpc=true"
                            ,(if aria2-rpc-token (format "--rpc-token=%s" aria2-rpc-token))
                            ,(format "--rpc-listen-port=%s" (aria2--port))
                            ,(format "--dir=%s" aria2-download-directory)
                            ,(format "--save-session=%s" aria2-session-file)
                            ,(if (file-exists-p aria2-session-file) (format "--input-file=%s" aria2-session-file)))))))
      (when aria2--debug
      ...
      (oset this pid (or (car (cl-remove-if-not #'aria2--is-aria-process-p (list-system-processes))) -1))
      (unless (is-process-running this)
                           ...
                           (json-serialize
                            `((jsonrpc . 2.0)
                              ...
                              (params . ,(vconcat
                                          (delq nil
                                               (append
                                                `(,(if aria2-rpc-token (format "token:%s" aria2-rpc-token)))
                                                params))))))
                           'utf-8))
...
(defvar aria2-mode-line-format
  (list
   ...
   " "
   (propertize
    (concat "[" (propertize "D" 'face 'aria2-modeline-key-face) "]:remove")
   ...
   " "
   (propertize
    (concat "[" (propertize "Q" 'face 'aria2-modeline-key-face) "]:terminate")
...
(defvar aria2-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map " " 'aria2-toggle-pause)
...
(defcustom aria2-mode-hook nil
  "Hook ran after enabling `aria2-mode'."
  :type 'hook
  :group 'aria2)
...
\\{aria2-mode-map}"
  :group 'aria2
  (unless (and aria2-executable (file-executable-p aria2-executable))
