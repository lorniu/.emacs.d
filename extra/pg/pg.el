...
;; TODO:
;; refer: https://github.com/brianc/node-postgres
...
(defconst pg:PG_PROTOCOL_MAJOR 2)  ; libpq/pgcomm.h
...
(cl-defstruct pgcon process pid secret (binaryp nil) encoding)
...
(cl-defmacro with-pg-connection ((con &rest open-args) &body body)
...
a result structure which can be decoded using `pg:result'."
  ...
  (let* ((last (car (last args)))
         (fmt-args (if (and last (listp last)) last))
         (sql (cl-loop for e in args when (stringp e) concat e))
         (tuples '())
         (attributes '())
         (result (make-pgresult :connection connection)))
    (if (and sql fmt-args)
        (setq sql (apply (apply-partially 'format sql)
                         (mapcar (lambda (a) (replace-regexp-in-string "\\(.\\)'\\(.\\)" "\\1''\\2" a))
                                 fmt-args))))
    ...
    (cl-loop for c = (pg:read-char connection) do
             (cl-case c
               ...
               (#x32 t) ; bindComplete
               (#x33 t) ; closeComplete
               (#x57 t) ; replicationStart
               (#x63 t) ; copyDone
               (#x69 t)
               (#x6e t) ; no data
               (#x73 t) ; portalSuspended
               (#x78 t)
               (#xff t)
...
(defun pg:read-char (connection)
  ...
  (let ((process (pgcon-process connection)))
    (with-current-buffer (process-buffer process)
      (unless (char-after 1)
        (pg::accept-process-output process 0.005)
...
(defun pg::process-send (process string)
  ...
  (cl-assert (not pg::inside-accept-process-output))
