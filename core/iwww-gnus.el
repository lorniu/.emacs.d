...
(setq gnus-secondary-select-methods
      `((nntp "news.gmane.io")
        (nntp "news.gwene.org")
        (nnatom "https://www.reddit.com/r/emacs/.rss" (disable)) ; reddit
        (nnimap "gmail"
...
(setq gnus-select-method '(nnnil nil)) ; legacy, not use
(setq gnus-secondary-select-methods
      (cl-remove-if (lambda (method)
                      (or (assoc 'disable method)
                          (and (eq (car method) 'nnimap) (null (lookup-password :host (cadr method))))))
...
(setq gnus-user-date-format-alist '(((gnus-seconds-today) . "Today %H:%M") ((gnus-seconds-year)  . "%m/%d %H:%M") (t . " %Y.%m/%d")))
(setq gnus-summary-line-format "%U%R%z%I%(%[ %&user-date;: %-21,21f %]%) %s\n")
;; article
...
(x gnus-start
   ...
   ;; hooks
   (setq gnus-activate-level 2) ; when startup, only fetch group leval <= 2, Use S l to change group level
...
(defun im/gnus-threads-setup ()
  ...
  (let* ((subtree (completing-read "Thread hide subtree: " '("yes" "no") nil t nil nil
    ...
    (setq gnus-thread-hide-subtree (equal subtree "yes"))
    (setq gnus-summary-thread-gathering-function (if (equal gathering "references")
                                                     #'gnus-gather-threads-by-references
                                                   #'gnus-gather-threads-by-subject))
    (setq gnus-thread-sort-functions (mapcar #'intern-soft sort))
...
(defun im/gnus-make-rss-group ()
  ...
  (interactive nil gnus-group-mode)
  (let ((url (completing-read "URL to Search for RSS: " ic/gnus-rss-urls)))
  ...
  [[("" (lambda () (!tdesc "^/B" "Server..")) ignore :format " %d")
    ("G" "  Group.." (lambda ()
                       ...
                       (im:execute-command-matching "Groups: "
                         '("^gnus-group.*\\(group$\\|edit-group\\|virtual$\\|directory$\\)" im/gnus-make-rss-group))))
    ("t" "  Topic.." (lambda ()
                       (interactive)
