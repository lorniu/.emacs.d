...
(setq gnus-select-method '(nnnil "") ; legacy, not use
      ...
      `((nntp "news.gmane.io")
        ...
        (nnimap "gmail"
                ;; machine gmail login EMAIL password PASSWORD port imaps
...
(x gnus-start
   ...
   ;; Keybinds
   (define-key gnus-group-mode-map (kbd "C-c m") 'ln/assist-gnus-group-mode)
   (define-key gnus-summary-mode-map (kbd "C-c m") 'ln/assist-gnus-summary-mode)
   (define-key gnus-article-mode-map (kbd "C-c m") 'ln/assist-gnus-article-mode)
   (define-key gnus-group-mode-map (kbd "G R") 'ln/gnus-make-rss-group))


;;; Helpers

(defun ln/gnus-group-toggle-level-number ()
...
(defun ln/gnus-make-rss-group ()
  "Subscribe RSS group from URL."
  (interactive nil gnus-group-mode)
  (let ((url (completing-read "URL to Search for RSS: " ln-gnus-rss-urls)))
    (gnus-group-make-rss-group (and (> (length url) 0) url))))

(defun ln/gnus-article-toggle-show-image ()
  (interactive nil gnus-group-mode)
...
(transient-define-prefix ln/assist-gnus-group-mode ()
  :transient-non-suffix 'transient--do-exit
  [[("" (lambda () (!tdesc "^/B" "Server..")) ignore :format " %d")
    ("G" "  Group.." (lambda ()
                       (interactive)
                       (ln:execute-command-matching "Groups: "
                         '("^gnus-group.*\\(group$\\|edit-group\\|virtual$\\|directory$\\)"
                           ln/gnus-make-rss-group
                           ln/gnus-group-toggle-level-number))))
    ("t" "  Topic.." (lambda ()
                       (interactive)
                       (ln:execute-command-matching "Topic: " "^gnus-topic" "sort")))]
   [("l" "  List.." (lambda ()
                      (interactive)
                      (ln:execute-command-matching "View list: "
                        '("^gnus-group.*\\(list\\|apropos\\)" gnus-topic-mode))))
    ("s" "  Sort.." (lambda ()
                      (interactive)
                      (ln:execute-command-matching "Sort: " "^gnus-.*sort")))
    ("L" "  Level.." (lambda ()
                       (interactive)
                       (ln:execute-command-matching "Group level: " "gnus-group.*level")))]
   ...
   [("" (lambda () (!tdesc "G G" "Query..")) ignore :format " %d")
    ...
    ("" (lambda () (!tdesc "s/u/#/F/R/ami, ln/gnus-make-rss-group, ln/gnus-threads-setup" "")) ignore :format " %d")
    ]]
  (interactive)
  (if (eq major-mode 'gnus-group-mode)
      (transient-setup 'ln/assist-gnus-group-mode)
    (user-error "You should invoke this in gnus-group-mode")))

(transient-define-prefix ln/assist-gnus-summary-mode ()
  :transient-non-suffix 'transient--do-exit
  [[("m" "Mark.." (lambda ()
                    (interactive)
                    (ln:execute-command-matching "Mark: "
                      "^gnus-summary.*\\(mark\\|tick\\|expire\\|kill\\)\\|gnus-cache.*art"
                      "put-mark\\|limit-")))
    ("/" "Limit.." (lambda ()
                     (interactive)
                     (ln:execute-command-matching "Limit to: " "^gnus-summary.*limit")))]
   [("s  " "Score.." (lambda ()
                       (interactive)
                       (ln:execute-command-matching "Score: "
                         '("^gnus-summary.*score"
                           gnus-score-edit-all-score
                           gnus-score-edit-current-scores
                           gnus-summary-raise-thread
                           gnus-summary-lower-thread))))
    ("v i" "Toggle Show Image"  ln/gnus-article-toggle-show-image)]
   [("" (lambda () (!tdesc "M-g | / N" "Refresh Topics.."))   ignore :format " %d")
    ("" (lambda () (!tdesc "A T | ^  " "All/Parent Article")) ignore :format " %d")]]
  (interactive)
  (if (eq major-mode 'gnus-summary-mode)
      (transient-setup 'ln/assist-gnus-summary-mode)
    (user-error "You should invoke this in gnus-summary-mode")))

(transient-define-prefix ln/assist-gnus-article-mode ()
  :transient-non-suffix 'transient--do-exit
  [[("r" "Reply.." (lambda ()
                     (interactive)
                     (ln:execute-command-matching "Send: "
                       '(gnus-summary-reply
  ...
  (if (eq major-mode 'gnus-article-mode)
      (transient-setup 'ln/assist-gnus-article-mode)
