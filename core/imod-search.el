...
(x replace
   ...
   (global-set-key (kbd "M-s o") ; Word at-point first
                   (defun ln/occur (str)
                     (interactive (list (ln:thing-at-region-or-point)))
...
(defun ln/grep-cursor (word)
  "Grep the current WORD in the files."
  (interactive (list (ln:thing-at-region-or-point 'word)))
...
(defcustom ln-rg-groups
  `(("e"   (loce ""))
    ("a"   agenda-directory)
    ("n"   org-directory)
    ("s"   ln-srcdir "ln-srcdir not config")
    ("w"   ln-workdir "ln-workdir not config")
    ("p"   (project-root (project-current)) "No project found"))
  "Groups used by `ln/rg-smart-search'. Format: (group-char search-dir prompt-when-non-exist*)+"
  :group 'imfine
  :type 'list)

(defvar ln:rg-info nil)
(defvar ln:rg-history nil)

(defun ln:rg-build-command (glob input &optional opts)
...
(defun ln:rg-parse-input (string)
  ...
  (let* ((beg 0) group input glob dir opts command)
    ...
    (with-temp-buffer
      (insert string)
      (goto-char (point-min))
      (when (looking-at (format "\\([%s]\\)\\(?: +\\|\\. *\\)" (mapconcat #'car ln-rg-groups "")))
      ...
      (if-let* ((dir-info (and group (cadr (assoc group ln-rg-groups)))))
    ...
    (setq command (ln:rg-build-command glob input opts))
    (list :origin string :input input :group group :beg beg :glob glob :dir dir :opts opts :command command)))

;; (ln:rg-parse-input "n.org group")
;; (plist-get ln:rg-info :dir)

(defun ln:rg-candicates (info)
  "Query via ripgrep.
Syntax: (ln:rg-candicates (list :input require.* :glob *.el :dir ...))
...
(defun ln:rg-dispatch-to-rg.el ()
  "Use rg.el to show the results in Occur buffer."
  (interactive)
  (ln:exit-minibuffer-with-message (c (ln:completion-compat :current))
    (let ((default-directory (plist-get ln:rg-info :dir)))
      ;; search and display in dedicated buffer
      (grep (plist-get ln:rg-info :command))
                     ...
                     (lambda (_a _b)
                       (when (string-match "\\`\\(.*?\\):\\([0-9]+\\):\\(.*\\)\\'" c)
                         (let ((file-name (replace-regexp-in-string
                                           "^[\\./]*" ""
                                           (cl-subseq (match-string-no-properties 1 c) (plist-get ln:rg-info :beg))))
                               (line-number (match-string-no-properties 2 c)))
                           (search-forward (format "%s:%s:" file-name line-number) nil t)
                           (beginning-of-line)
                           (when (re-search-forward (plist-get ln:rg-info :input) (line-end-position) t)
                             (goto-char (match-beginning 0)))))))))
      ;; save input to history
      (when-let* ((item (plist-get ln:rg-info :origin)))
        (ln:history-replace 'ln:rg-history item)))))

(defun ln/rg-smart-search ()
  "Search with ripgrep smartly.

Default search current directory, if input begin with 'p ' then search current project,
if begin with 'n ' then search org-directory. See `ln-rg-groups' for other groups.
...
Make sure package `rg.el' is installed."
  ...
  (let* ((completion-styles '(orderless)) ; only support orderless style
         (prompt (format "Candidates [%s]: " (string-replace " " "" (mapconcat #'car ln-rg-groups ""))))
         (word (ln:thing-at-region-or-point
                (lambda ()
                  ...
                  (cond
                   ...
                   (t (condition-case err
                          (let* ((text (minibuffer-contents-no-properties)))
                            (if (and (equal text (plist-get ln:rg-info :origin)) (plist-get ln:rg-info :items))
                                (complete-with-action action (plist-get ln:rg-info :items) "" nil)
                              (let* ((completion-regexp-list nil)
                                     (info (ln:rg-parse-input text))
                                     (items (progn
                                              (when (plist-get info :group)
                                                (add-face-text-property (minibuffer-prompt-end) (+ 1 (minibuffer-prompt-end)) 'warning))
                                              (ln:rg-candicates info))))
                                (setq ln:rg-info (plist-put info :items items))
                                (unless (plist-get info :items) (error "Nothing found..."))
                                (complete-with-action action (plist-get ln:rg-info :items) "" nil))))
                        (error (setq ln:rg-info nil) (cdr err)))))))
         (line (catch 'miniquit
                 (minibuffer-with-setup-hook
                     (lambda ()
                       (let ((text (minibuffer-contents-no-properties)))
                         (unless (equal text (plist-get ln:rg-info :origin))
                           (setq ln:rg-info (ln:rg-parse-input text))))
                       (use-local-map (make-composed-keymap nil (current-local-map)))
                       (local-set-key (kbd "M-w")
                                      (lambda ()
                                        (interactive)
                                        (when-let* ((c (ignore-errors (cadr (split-string (ln:completion-compat :current) "[0-9]:")))))
                                          (kill-new c)
                                          (throw 'miniquit 'Copy))))
                       (local-set-key (kbd "C-c o") #'ln:rg-dispatch-to-rg.el)
                       (local-set-key (kbd "C-c C-o") #'ln:rg-dispatch-to-rg.el))
                   (completing-read prompt table nil nil word 'ln:rg-history)))))
    (if (symbolp line) (user-error "%s done" line))
    (when-let* ((h (plist-get ln:rg-info :origin))) ; history item
      (ln:history-replace 'ln:rg-history h))
    (if (string-match (format "\\`\\(.*?\\):\\([0-9]+\\):\\(.*\\)\\'") line)
        (let ((file-name (match-string-no-properties 1 line))
          ...
          (setq isearch-string (plist-get ln:rg-info :input))
          (isearch-update-ring isearch-string t)
          (find-file (expand-file-name file-name (plist-get ln:rg-info :dir)))
          (goto-char (point-min))
