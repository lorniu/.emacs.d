  ...
  (let* ((beg 0) group input glob dir opts command)
    (with-temp-buffer
      ...
      (when-let* ((pos (cl-search " --- " string)))
        (setq opts (substring string (+ 5 pos))
              string (substring string 0 pos)))
      (insert string)
      (goto-char (point-min))
      ;; Parse group prefix
      (when (looking-at (format "\\([%s]\\)\\(?:[ .]\\)" (mapconcat #'car im.rg-groups "")))
        (setq group (match-string 1)
              beg (1- (goto-char (match-end 0)))))
      ;; Parse glob pattern
      (when (looking-at "\\.\\([^ ]+\\) *")
        (setq glob (match-string 1)
              beg (1- (goto-char (match-end 0))))
        (cond ((string-match-p "^[a-z][a-z0-9]+$" glob)
               (setq glob (concat "*." glob)))
              ((string-match-p "^[a-zA-Z0-9_-]+/$" glob)
               (setq glob (format "**/*%s*/**" (substring glob 0 -1))))))
      ;; Extract search input
      (when (>= (- (length (encode-coding-string string 'utf-8)) beg) 3)
        (setq input (substring string beg)))
      ;; Resolve directory
      (setq dir (if-let* ((info (and group (cadr (assoc group im.rg-groups)))))
                    (eval info)
                  default-directory))
      (unless (file-exists-p dir)
        (user-error "Directory not exists: %s" dir))
      (when (member (file-name-as-directory dir)
                    (list "/" "C:/" "D:/" (file-name-as-directory (getenv "HOME"))))
        (user-error "Cannot search from root or home directory"))
      ;; Build rg command
      (setq command (grep-expand-template
                     (format "rg --no-heading --no-config %s %s-n -z -e <R> ."
                             (or opts "-S")
                             (if glob (format "-g '%s' " glob) ""))
                     (rxt-elisp-to-pcre (or input ""))))
      ;; Ret
...
(defun im:rg-candicates (info)
  "Query via ripgrep: (im:rg-candicates (list :input require.* :glob *.el :dir ...)"
  (when (zerop (length (plist-get info :input)))
    (user-error "Please input search pattern..."))
  (let* ((default-directory (plist-get info :dir))
    ...
    (dolist (line lines lines)
      (when (string-match "\\`\\([^:]+\\):\\([^:]+\\):" line)
        (add-face-text-property (match-beginning 1) (match-end 1) 'compilation-info nil line)
...
Make sure package `rg.el' is installed."
  ...
  (let* ((completion-styles '(orderless)) ; only support orderless style
         ...
         (table (lambda (_input _pred action)
                  (pcase action
                    (`metadata '(metadata (category . ripgrep)
         ...
         (copy-line
          (lambda ()
            (interactive)
            (run-at-time 0 nil
                         (lambda (c) (when c (kill-new c) (message "Copy done.")))
                         (ignore-errors (cadr (split-string (im:completion-compat :current) "[0-9]:"))))
         ...
         (dispatch-to-rg.el
          (lambda ()
            ...
            (abort-recursive-edit)))
         (line (minibuffer-with-setup-hook
                   (lambda ()
                     (let ((text (minibuffer-contents-no-properties)))
                     ...
                     (local-set-key (kbd "M-w") copy-line)
                     (local-set-key (kbd "C-c o") dispatch-to-rg.el)
                     (local-set-key (kbd "C-c C-o") dispatch-to-rg.el))
    ...
    (if (string-match (format "\\`\\(.*?\\):\\([0-9]+\\):\\(.*\\)\\'") line)
        (let ((file-name (match-string-no-properties 1 line))
          ...
          (dolist (ov (overlays-at (point)))
            (funcall (or (overlay-get ov 'isearch-open-invisible) #'ignore) ov))
          (recenter)
