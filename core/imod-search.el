...
(x rg
   "pacman -S ripgrep"
   :ref ("dajva/rg.el" "ripgrep download page: BurntSushi/ripgrep/releases")
   :config
   (setopt rg-group-result nil
           rg-ignore-case 'smart
           rg-align-column-number-field-length 4)
   (define-key rg-mode-map "G" 'im/rg-rerun-toggle-group))

(defun im/rg-rerun-toggle-group ()
  "Toggle show group in `rg-cur-search`."
  (interactive)
  (setopt rg-group-result (not rg-group-result))
  (rg-rerun))
