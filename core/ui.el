...
;;
;;   (setq im.faces
;;         `((base :title "imfine") ; inherited by others
;;           (aaa :font "LXGW WenKai Mono 17"
...
(defun f/get (item &optional default)
  (when (and im.faces (atom (car im.faces)))
    (setq im.faces (list (cons 'base im.faces))))
  (let* ((target (cdr (or
                       ;; search via enviorment variable $face as index
                       (and (string-match-p "^[0-9]+$" (concat (getenv-internal "face")))
                            (nth (string-to-number (getenv-internal "face")) im.faces))
                       ;; the first key with some special flags
                       (cl-find-if (lambda (item)
                                     (string-match-p "[.*~-]$" (format "%s" (car item))))
                                   im.faces)
                       ;; the first config
                       (car im.faces))))
         ;; the base section is special, for global settings
         (base (cdr (assoc 'base im.faces)))
         (faces (if (eq base target) target (im:plist-merge base target))))
    (cond
    ...
    ('*preset*
     (let ((preset (f/get (or value 'def))))
       ...
       (let ((fontset (face-attribute 'default :fontset)))
         ...
         (set-fontset-font fontset 'cjk-misc (plist-get preset :font-cn))
...
(defun f/fontset-font (fontset charset font)
  ...
  (interactive (let* ((fs (or f/current--fontset
                      ...
                      (items (mapcar #'symbol-name '(zh emoji symbol unicode han cjk-misc)))
                      (rg (ignore-errors (read (completing-read "Range to set: "
                                                                (lambda (input pred action)
                                                                  (if (eq action 'metadata)
                                                                      `(metadata (display-sort-function . ,#'identity)) ; affixation/annotation-function
...
(setq-default frame-title-format
              (f/get :title
                     `((:eval (cond ((equal (elt (buffer-name) 0) ? ) "Emacs")
                                    (t "Emacs =>  %b"))))))
