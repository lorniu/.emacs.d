...
;;
;;   (setq ln-faces '(:height 200 :font "xxx")) ; for simple
;;
;; Notice:
;;
...
(defun f/get (item &optional default)
  (when (and ln-faces (atom (car ln-faces)))
    (setq ln-faces (list (cons 'default ln-faces))))
  (let ((faces (cdr (or (and (string-match-p "^[0-9]+$" (concat (getenv-internal "face")))
                             (nth (string-to-number (getenv-internal "face")) ln-faces))
    ...
    (cond
     ...
     ((not (keywordp item)) (cdr (assoc item ln-faces)))
     (t (let* ((faces-default (list :title nil :theme nil :font nil))
               (faces-merged (ln:plist-merge faces-default faces))
...
(defun f/setup (param value)
                      ...
                      (value (cond
                              ...
                              ((eq param 'weight)
                               (intern (completing-read (format-prompt "New value of %s" (frame-parameter nil 'weight) param)
                                                        (ln:completion-table '(thin extra-light light semi-light normal medium semi-bold bold ultra-bold heavy ultra-heavy)))))
                              ((eq param '*preset*)
                               (intern (completing-read "Preset: " ln-faces #'cdr t)))
  ...
  (pcase param
    ...
    ('indent-width/offset (call-interactively #'ln/set-indent-width-or-offset))
    ((or 'height 'weight)
...
(when IS-NG
  ...
  (unless (f/get :theme) (setf (plist-get (cdar ln-faces) :theme) 'origin))
  (xterm-mouse-mode)
