;;; patch-haskell.el --- Patcher for haskell -*- lexical-binding: t -*-
;;; Commentary:
;;; Code:
...
(defun haskell-process-type ()
  (let ((cabal-sandbox (locate-dominating-file default-directory "cabal.sandbox.config"))
    ...
    (if (eq 'auto haskell-process-type)
        (cond ((and cabal-sandbox (executable-find "cabal")) (setq inferior-haskell-root-dir cabal-sandbox) 'cabal-repl)
...
(defun haskell-process-load-complete (session process buffer reload module-buffer &optional cont)
  (when (get-buffer (format "*%s:splices*" (haskell-session-name session)))
    (with-current-buffer (haskell-interactive-mode-splices-buffer session) (erase-buffer)))
  (let* ((ok (cond ((haskell-process-consume process "Ok,\\(?:.+\\) modules? loaded\\.$") t)
    ...
    (when cont (condition-case-unless-debug e (funcall cont ok) (error (message "%S" e)) (quit nil)))))

(defun haskell-mode-find-def (ident)
  (when (stringp ident)
    (let ((reply (haskell-process-queue-sync-request (haskell-interactive-process) (format (if (string-match "^[a-zA-Z_]" ident) ":info %s" ":info (%s)") ident))))
          ...
          (let ((defined (match-string 2 reply)))
            (let ((match (string-match "\\(.+?\\):\\([0-9]+\\):\\([0-9]+\\)$" defined)))
              (if match (list 'file
                              ...
                              (string-to-number (match-string 2 defined)) (string-to-number (match-string 3 defined)))
                (let ((match (string-match "‘\\(.+?\\):\\(.+?\\)’$" defined)))
                  (if match (list 'library (match-string 1 defined) (match-string 2 defined))
                    (let ((match (string-match "‘\\(.+?\\)’$" defined)))
                      (if match (list 'module (match-string 1 defined))))))))))))))


(provide 'patch-haskell)

;;; patch-haskell.el ends here
