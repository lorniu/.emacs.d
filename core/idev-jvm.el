...
(with-eval-after-load 'cc-mode
  (c-add-style "java-9" '("java"
                          ...
                          (c-offsets-alist
                           (case-label . +)
                           (inexpr-class . 0)
                           (arglist-cont-nonempty . (ln:arglist-cont-lambda c-lineup-gcc-asm-reg c-lineup-arglist))
                           )))

  (defun ln:arglist-cont-lambda (_arg)
...
(x android-mode
   ...
   (cl-loop for h in '(java-mode-hook groovy-mode-hook nxml-mode-hook)
            do (add-hook h #'ln:maybe-android-h)))
