...
(defun ln:loc-agenda-file (name &optional nocheck)
  (let ((f (expand-file-name name agenda-directory)))
    (if (or nocheck (file-exists-p f)) f
      (expand-file-name "notes.org" agenda-directory))))

(defvar bbdb-file                (ln:loc-agenda-file ".bbdb" t))
(defvar diary-file               (ln:loc-agenda-file ".diary" t))
(defvar org-default-notes-file   (ln:loc-agenda-file "notes.org"))
(defvar org-default-tasks-file   (ln:loc-agenda-file "tasks.org"))
(defvar org-default-links-file   (ln:loc-agenda-file "links.org"))
(defvar org-default-words-file   (ln:loc-agenda-file "words.org"))
(defvar org-default-working-file (ln:loc-agenda-file "working.org"))
(defvar org-default-journal-file (ln:loc-agenda-file "z-journal.org"))
...
(setq org-agenda-skip-deadline-if-done t
      ...
      org-show-notification-handler #'ln:org-notify-with-alert)
...
(setq org-capture-templates
      `(("j" "Journal" plain (file+function ,org-default-journal-file org-reverse-datetree-goto-date-in-file) "--- %<%T> ---\n\n%i%?" :empty-lines 1 :trim t)
        ("t" "Task" entry (file+function ,org-default-tasks-file ln:org-capture-interactive) "* TODO %u %i%?" :prepend t :jump-to-captured t)
        ("w" "New Word" item (file+function ,org-default-words-file ln:org-capture-interactive) "%i%?" :prepend t :jump-to-captured t)
        ("l" "Favor Link" plain (file+function ,org-default-links-file ln:org-capture-interactive) "%i%?" :prepend t :jump-to-captured t)
        ("n" "Draft/Tips" entry (file ,org-default-notes-file) "* %U\n\n%i%?" :prepend t :empty-lines 1)
        ("N" "Draft/Tips*" plain (file+function ,org-default-notes-file ln:org-capture-interactive) "%i%?" :jump-to-captured t)
        ("a" "Agenda..." entry (function ln:org-capture-interactive) "* %i%?" :prepend t :jump-to-captured t)))
...
(x org
   :bind (:map org-mode-map
               ("×"       . "*")
               ("C-c \""  . ln/org-edit-special-with-wc-setup)
               ("M-."     . org-open-at-point)
   ...
   ;; fixup clock
   (run-with-idle-timer 10 nil (lambda () (ln:with-message nil (require 'org-clock) (org-clock-load)))))

(x ox
   :config
   (require 'ox-spectacle)
   (add-to-list 'org-export-filter-final-output-functions
                (defun ln:org-publish-remove-zero-width-spaces-maybe (text _backend _info)
...
(defun ln:org-completing-read-headline (&optional org-file)
  ...
  (let* ((completion-ignore-case t)
    ...
    (string-trim (completing-read "Headline: " (ln:completion-table heads)))))

(defun ln:org-capture-interactive ()
  ...
  (let ((file (car-safe (cdr-safe (org-capture-get :target)))))
    (if (and (stringp file) (file-exists-p file)) ; file+function
        (progn
          ...
          (let ((headline (ln:org-completing-read-headline)))
      ...
      (let ((org-agenda-files (directory-files agenda-directory t "\\.org$"))) ; function
        ...
        (ln:org-show-plain-text-and-children-headlines)))))

(defun ln:org-show-plain-text-and-children-headlines ()
  (org-fold-show-entry)
  (org-with-limited-levels (org-fold-show-children))
  (org-fold-show-set-visibility 'tree))

(defun ln:org-in-drawer-p ()
...
(defun ln/org-align-all-tables ()
  (interactive nil org-mode)
  (org-table-map-tables 'org-table-align 'quietly))

(defun ln/org-edit-special-with-wc-setup ()
  (interactive nil org-mode)
...
(defun ln/org-update-id-locations ()
...
(defun ln/org-toggle-emphasis-markers ()
  (interactive nil org-mode)
...
(defun ln/org-agenda-lines ()
  "Read and goto headline in `org-agenda-files'."
  (interactive)
  (consult-org-agenda "+LEVEL<3+ITEM<>{^[<\\[]}-Archived-elfeed-Config")
  (ln:org-show-plain-text-and-children-headlines))

(defun ln/org-agenda-files ()
  "Fast open file in `org-agenda-files'."
  (interactive)
  (let ((f (completing-read "Agenda files: " (ln:completion-table org-agenda-files nil "file") nil t)))
    (find-file f)
    (when (= (point) 1)
...
(defun ln/org-publish-clear-timestamp-directory ()
  (interactive)
...
(defun ln/org-protocol-enable-for-linux ()
  (interactive)
  (cl-assert IS-LINUX)
  (let ((path
    ...
    (if (file-exists-p path) (message "%s already exists." path)
      (with-current-buffer (find-file path)
        (insert (ln:string-indent content)))
...
(x ob-plantuml
   ...
   (defun ln/download-plantuml-jar ()
     (interactive)
...
(transient-define-prefix ln/transient-agenda () ; C-c c
  [:hide
   (lambda () t)
   ("m"   "1"  ln/note-publish-interactively)
   ("N"   "2"  (lambda () (interactive) (ln/note-publish t)))
   ("M"   "3"  (lambda () (interactive) (ln/note-publish-interactively t)))
   ("R"   "4"  ln/note-publish-reset-directories)
   ...
   ("W"   "7"  ln/view-freshwords)]
  [["Agenda"
    ("l"  "Agenda Lines"   ln/org-agenda-lines)
    ("f"  "Agenda Files"   ln/org-agenda-files)]
   ["Notes"
    ("  p"  "   Capture"  org-capture)
    ("n" (lambda () (!tdesc "nN mMR"  " Publish..")) ln/note-publish :format "%d")]
   [:description
   ...
   ["Miscellaneous"
    ...
    ("w" (lambda () (!tdesc "wWp" "Words")) ln/pick-freshword :format "%d")]
   ]
  (interactive)
  (require 'ox-publish)
  (transient-setup 'ln/transient-agenda))

(transient-define-prefix ln/assist-org-mode () ; C-c m
  [:hide
   (lambda () t)
   ("s"    "1"   org-schedule)
  ...
  [["Schedule/TimeIt"
   ...
   ["View"
    ...
    ("v m"   "Toggle Emphasis-Markers" ln/org-toggle-emphasis-markers)
  ...
  (if (eq major-mode 'org-mode)
      (transient-setup 'ln/assist-org-mode)
