...
(x org
   ...
   ;; babel
   (cl-loop for lang in '( shell powershell typescript
                           lisp python ruby haskell java js csx csharp fsharp
                           sql sqlite
                           gnuplot ditaa dot plantuml calc
                           restclient latex awk )
            unless (eq (elt (symbol-name lang) 0) ?-) collect `(,lang . t) into lst
   ...
   (setq org-emphasis-regexp-components
         '("-[:multibyte:][:space:]('\"{"           ; prematch
           "-[:multibyte:][:space:].,:!?;'\")}\\["  ; postmatch
...
(defun im:org-completing-read-headline (&optional org-file)
  ...
  (let* ((completion-ignore-case t)
    ...
    (string-trim (completing-read "Headline: " (im:ordered-completion-table heads)))))
...
(transient-define-prefix im/assist-org-mode ()
  [:hide
   (lambda () t)
   ("s"    "1"   org-schedule)
