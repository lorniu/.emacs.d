...
Item should be (win-id-list window-configuration buffer-names updated-at) style.")
...
(defun im/toggle-layout (&optional arg)
  ...
  (cl-labels
      ((win-ids ()
       ...
       (save-layout ()
         (let ((ids (win-ids)))
           (setq im:fullscreen-alist
                 (cl-remove-if (lambda (c) (equal (car c) ids)) im:fullscreen-alist))
           (push (list ids
       ...
       (switch-layout ()
         (setq im:fullscreen-alist
               (cl-remove-if (lambda (c) (cl-notany #'get-buffer (caddr c))) im:fullscreen-alist))
         (cl-labels
           ...
           (let* ((current-ids (win-ids))
                  (cands (mapcar
                          (lambda (wc)
                            (let* ((ids (car wc))
                                   (idstr (ids-str ids))
                                   (is-current (equal current-ids ids))
                                   (dts (propertize
                                         (format "%-35s" (format-seconds "%Y %D %H %M %z%S" (- (float-time) (cadddr wc))))
                                         'face 'font-lock-string-face))
                                   (bufs (mapconcat
                                          (lambda (name)
                                            (if (get-buffer name) name
                                              (propertize name 'face '(:strike-through t))))
                                          (caddr wc)
                                          ", ")))
                              (when is-current
                                (setq idstr (propertize idstr 'face 'font-lock-comment-face)))
                              (add-face-text-property 0 (length bufs) 'font-lock-comment-face nil bufs)
                              (cons (concat idstr (make-string (- 25 (length idstr)) ? ) dts bufs) wc)))
                          im:fullscreen-alist))
                  (chosen (if (length= cands 1)
                              (caar cands)
             ...
             (pcase chosen
               ('del (im/toggle-layout 'reopen))
               ('clear (message "Clear done."))
               (_ (if-let* ((wc (cadr (cdr (assoc chosen cands))))) (set-window-configuration wc))))))))
