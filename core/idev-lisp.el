...
(defvar ln-lisp-dev 'sly)
(defvar ln-lisp-init-file (loce "share/lisp/init.lisp"))

(prog1 'assert-lisp-env
  (defun %lisp/devenv-nofound ()
    (if ln-lisp-dev (message "`ln-lisp-dev' is `%s', but not found in `load-path'." ln-lisp-dev)
      (message "Please setup `ln-lisp-dev' correctlly.")))
  (when ln-lisp-dev
    (condition-case _err
        (load (format "%s-autoloads" ln-lisp-dev) nil t)
   ...
   (pcase ln-lisp-dev
     ('slime
      ...
      (with-eval-after-load 'slime
        ...
        (defun:hook slime-connected-hook () (ln:load-lisp-init-file))))
     ...
     ('sly
      ...
      (defun:hook sly-connected-hook () (ln:load-lisp-init-file))
...
(cl-macrolet ((define-lisp-implementations (&rest decl)
                (macroexp-progn
                 (cl-loop
                  with var = (intern (format "%s-lisp-implementations" ln-lisp-dev))
                  for (symbol . args) in (cl-remove-if (lambda (x) (not (executable-find (caadr x)))) decl)
                  for funame = (intern (format "%s/%s" ln-lisp-dev symbol))
                  collect `(defun ,funame () (interactive) (,ln-lisp-dev ',symbol)) into fs
                  collect `(,symbol ,@args) into hs
                  finally return (append fs (list `(with-eval-after-load ln-lisp-dev (setq ,var (append ',hs ,var)))))))))
  (define-lisp-implementations
   (ros  ("ros" "run"))
   (ccl  ("ccl"))
   (sbcl ("sbcl"))))

(defun ln:load-lisp-init-file ()
  (when (file-exists-p ln-lisp-init-file)
    (funcall
     (pcase ln-lisp-dev
     ...
     `(cl:unless
       ...
       (cl:load (cl:merge-pathnames ,ln-lisp-init-file))))))

(defun ln/ensure-quicklisp ()
  (interactive)
  (let ((url "http://beta.quicklisp.org/quicklisp.lisp")
    ...
    (unless (file-exists-p file)
      ...
      (with-temp-buffer
        ...
        (pcase ln-lisp-dev ('sly (sly-eval-buffer)) ('slime (slime-eval-buffer)))))))
...
(defun ln/dispatch-macroexpand (&optional arg)
...
(defun ln/toggle-lisp-indent-style-locally ()
...
(transient-define-prefix ln/assist-debugger-mode ()
  ...
  (if (eq major-mode 'debugger-mode)
      (transient-setup 'ln/assist-debugger-mode)
