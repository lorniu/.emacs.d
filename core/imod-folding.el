...
;; - hide-comnt.el, make comments invisible, I prefer `ln/hs-hide-all-comments'

;;; Code:

(defface ln/hs-ellipse-face '((t :inherit warning))
  "Hideshow ellipse."
  :group 'imfine)

(defface ln/outline-ellipse-face '((t :weight thin))
  "Outline ellipse."
  :group 'imfine)
     ...
     (let ((display-table (or buffer-display-table (make-display-table))))
       ...
       (set-display-table-slot display-table 4
                               (vconcat (mapcar (lambda (c)
                                                  (make-glyph-code c 'ln/outline-ellipse-face))
                                                "..."))))))
...
(x hideshow/e
   ...
   :bind ( :map imfine-mode-map
           ("C-c f"   . ln/smart-folding)
           ("C-c F"   . ln/smart-folding-all)
           ("C-c M-f" . (lambda () (interactive) (ln/smart-folding-all 1)))
           ("C-c C-f" . ln/transient-fold)
                   ...
                   (overlay-put ov 'display
                                (concat pre
                                        (propertize
                                         ...
                                         'face 'ln/hs-ellipse-face 'cursor t 'pointer 'hand)))
...
(defun ln/smart-folding-all (&optional arg)
  (interactive "P")
  (cond
   ...
   ((derived-mode-p '(markdown-mode outline-mode emacs-news-mode eshell-mode))
    (call-interactively 'ln/outline-toggle-all))
   ((or hs-minor-mode outline-minor-mode)
    (cond ((and outline-minor-mode outline-prefer-p)
           (call-interactively 'ln/outline-toggle-all))
          (hs-minor-mode
...
(defun ln/hs-hide-level-and-comments ()
  (interactive)
  (let ((hs-allow-nesting t))
    (call-interactively 'ln/hs-hide-all-comments)
    (call-interactively 'hs-hide-level)))

(defun ln/outline-toggle-all ()
  (interactive)
...
(defun ln/outline-toggle-narrow ()
  (interactive)
...
(defun ln/outline-toggle-prefer ()
  (interactive)
...
(transient-define-prefix ln/transient-fold () ; C-c C-f
  [:hide
   (lambda () t)
   ("s"     "" hs-show-block)
   ("h"     "" hs-hide-block)
   ("l"     "" hs-hide-level)
   ("L"     "" ln/hs-hide-level-and-comments)
   ("c"     "" ln/hs-hide-all-comments)

   ("F"     "" ln/outline-toggle-all)
   ("N"     "" ln/outline-toggle-narrow)
  ...
  [[:description
   ...
   [""
    ("E" (lambda () (!tdesc "  E/P  " " Set-Outline")) ln/outline-regexp-this :format "%d")
...
(transient-define-prefix ln/transient-narrow () ; C-x n
  [:hide
   (lambda () t)
  ...
  (let ((transient-show-popup -0.3))
    (transient-setup 'ln/transient-narrow)))
