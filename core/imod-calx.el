...
(defun ln:cal-chinese-to-absolute (y m d)
  "将农历转换为绝对日期."
  (require 'cal-china)
  ...
  (calc-wrapper
   (cl-letf (((symbol-function #'math-parse-date-validate)
              (lambda (y big m d &rest _r)
                ...
                (let* ((ab (ln:cal-chinese-to-absolute y (if big m (+ 0.0 m)) d))
...
(defun:around calc//with-grab-region (f &rest args)
  ...
  (let (init)
    (when (use-region-p)
      (let ((text (ln:thing-at-region-or-point)))
...
(defun:around calc-do-quick-calc//quick-calc-with-region (f &rest args)
  (let ((quick-calc-init-string (ln:thing-at-region-or-point "")))
    (apply f args)))
...
(defun ln:get-calc-keymap-bindings (prefix &optional filter)
...
(transient-define-prefix ln/assist-calc-mode ()
  :transient-non-suffix 'transient--do-exit
  [[("SPC" "Top.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Top: "
         `( "calc.*-quick"
            "calc.*-truncate-"
            calc-insert-register calc-copy-to-register
            calc-clean-num calc-select-part
            ,@(ln:get-calc-keymap-bindings nil)))))
    (" h " "Help.." calc-full-help)
    (" m " "Modes.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Modes: "
         (ln:get-calc-keymap-bindings "m") "depth$")))
    (" T " "Trail.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Trail: " "^calc-trail-" "mode$")))]
   [("d r" "Disp Radix.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Radix: " "^calc.*-radix$")))
    ("d n" "Disp Notation.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Notation: " "^calc.*-notation$")))
    ("d l" "Disp Language.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Language: " "^calc.*-language$")))
    ("d d" "Disp Other.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Display: "
         (ln:get-calc-keymap-bindings "d" (lambda (_ d) (not (string-match-p "-radix$\\|-notation$\\|-language$\\|-redo$" d)))))))]
   [("f" "Functions.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Functions: "
         `( calc-abs calc-idiv calc-floor calc-trunc calc-ceiling
            calc-sqrt calc-exp calc-ln calc-ln calc-log10 calc-log
            ,@(ln:get-calc-keymap-bindings "f")))))
    ("r" "Trigonometric.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Trigonometric: "
         '(calc-pi
    ...
    ("p" "Programmability.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Programmability: "
    ...
    ("k" "Combinatorics/Statistics.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Combinatorics/Statistics: "
         `(calc-factorial ,@(ln:get-calc-keymap-bindings "k"))
         "keep-args$")))]
   [("b" "2#101/business.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "binary/business: "
         `(calc-percent calc-convert-percent ,@(ln:get-calc-keymap-bindings "b")))))
    ("t" "<1970/1/1>.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Time: "
         `("hms" calc-time calc-calendar calc-chinese-date calc-date-notation ,@(ln:get-calc-keymap-bindings "t"))
         "-trail\\|-tan\\|-quick")))
    ("v" "[vector/matrix].."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "vector/matrix: "
         `(calc-conj calc-concat ,@(ln:get-calc-keymap-bindings "v")))))
    ("u" "[Statistics].."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Statistics: "
         (ln:get-calc-keymap-bindings "u" (lambda (_ y) (string-match-p "vector" (format "%s" y)))))))]
   [("a" "Algebra.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Algebra: "
         (ln:get-calc-keymap-bindings "a")
         '("-abs$"
           calc-equal-to calc-remove-equal calc-not-equal-to
           calc-less-than calc-greater-than calc-less-equal calc-greater-equal
           calc-in-set))))
    ("c" "Convert.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Convert: "
         `(calc-floor calc-round ,@(ln:get-calc-keymap-bindings "c")) "-cos$")))
    ("U" "Units.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Units: "
         (ln:get-calc-keymap-bindings "u") "-vector\\|-undo")))
    ("l" "Log Units.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Log Units: "
         (ln:get-calc-keymap-bindings "l"))))]
   [("s" "Store/Recall.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Store/Recall: "
         (ln:get-calc-keymap-bindings "s"))))
    ("j" "Select.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Select: "
         (ln:get-calc-keymap-bindings "j") "conj$")))
    ("g" "Graphics.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Graphics: "
         (ln:get-calc-keymap-bindings "g") "argument$")))
    ("z" "Define/User.."
     (lambda ()
       (interactive)
       (ln:execute-command-matching "Define/User: " (ln:get-calc-keymap-bindings "Z"))))]]
  (interactive)
  (if (eq major-mode 'calc-mode)
      (transient-setup 'ln/assist-calc-mode)
