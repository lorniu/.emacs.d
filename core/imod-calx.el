;;; imod-calx.el --- calendar/calc -*- lexical-binding: t -*-
...
(x calc
   ...
   :init
   (setq calc-multiplication-has-precedence nil) ; division and multiplication have same precedence
   (setq calc-date-format '(YYYY "-" MM "-" DD " " Www (" " hh ":" mm ":" ss) CN)))

(defvar calc-date-format-history
  `(,calc-date-format
...
(transient-define-prefix im/transient-calc-mode ()
  ...
  (if (eq major-mode 'calc-mode)
      (transient-setup 'im/transient-calc-mode)
    (user-error "You should invoke this in calc-mode")))

(provide 'imod-calx)

;;; imod-calx.el ends here
