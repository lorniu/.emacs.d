...
(defcustom org-directory (ln:the-file "~/.notes/" "~/org")
  "Main Directory for Agenda/Notes/Diary and others."
  :type 'directory)

(defcustom ln-srcdir "~/source/"
  "Source directory."
  :type 'string)

(defcustom ln-workdir "~/workdir/"
  "Personal work directory."
  :type 'string)

(defcustom ln-downdir (ln:the-file (expand-file-name "Downloads/" (getenv (if IS-WIN "USERPROFILE" "HOME"))))
  "Directory used to download."
  :type 'string)

(defcustom ln-host "127.0.0.1"
  "My remote server. Ip or host."
  :type 'string)

(defcustom ln-host-user "vip"
  "Default username of remote server."
  :type 'string)

(defcustom ln-backup-dir (locc "backup-files")
  "Directory used to backup files."
  :type 'file)

(defcustom ln-auto-save-dir (locc "auto-save")
  "Directory used to auto save files."
  :type 'file)

(defun ln:host (&optional user host) (concat (or user ln-host-user) "@" (or host ln-host)))

(defcustom ln-external-paths (list (loco "bin"))
  "Extend the PATH environment."
  :type '(repeat string))

(defcustom ln-external-load-path-roots (list (loco "share/emacs/"))
  "Extra load-path root."
  :type '(repeat string))

(defcustom ln-system-terminal (cond (IS-WIN "start cmd %s")
...
(setq vc-make-backup-files t  ; backup versioned files
      ...
      backup-directory-alist `(("." . ,ln-backup-dir)))

(setq auto-save-default t ; save #periodically# to avoid crashes
      auto-save-interval 300
      auto-save-timeout 30
      auto-save-list-file-prefix (expand-file-name ".saves-" ln-auto-save-dir))
...
(cl-loop with ds = (list (loce "core") (loce "extra") (loce "repo") (loce "site-lisp"))
         for root in (append ln-external-load-path-roots ds)
...
(cond (IS-WIN   (add-to-list 'ln-external-paths (loce "bin/win")))
      (IS-MAC   (add-to-list 'ln-external-paths (loce "bin"))
                (add-to-list 'ln-external-paths (loce "bin/mac"))
                (add-to-list 'ln-external-paths "/opt/homebrew/bin"))
      (IS-LINUX (add-to-list 'ln-external-paths (loce "bin"))
                (add-to-list 'ln-external-paths (loce "bin/nix"))))

(dolist (d ln-external-paths exec-path)
...
(defvar ln-find-file-readonly-regexp nil)

(defun:hook find-file-hook/readonly ()
  "Files that should be readonly."
  (let* ((case-fold-search nil)
         (match? (lambda (&rest items) (string-match-p (apply #'ln:join-as-regor-group items) buffer-file-name))))
    (when (and buffer-file-name
               (or (and ln-find-file-readonly-regexp
                        (string-match-p ln-find-file-readonly-regexp buffer-file-name))
                   (and (not
...
(defun ln/local-assist ()
  "Run the command define by `ln:local-assist' method."
  (interactive)
  (ln:local-assist major-mode))

(cl-defmethod ln:local-assist (_default)
  "The general assist actions for current major mode."
  (let ((bn (intern (format "ln/assist-%s" (buffer-name)))))
    (if (commandp bn)
        (call-interactively bn)
      (let ((tn (intern (format "ln/assist-%s" major-mode))))
        (if (commandp tn)
...
(defcustom ln-custom-file-directory (loco "share/emacs/inits/")
  "Directory of custom-file."
  :group 'imfine
  :type 'file)

(defcustom ln-custom-file-file-name
  (format "init-%s@%s.el"
...
(defvar custom-file
  (if (file-exists-p ln-custom-file-directory)
      (expand-file-name ln-custom-file-file-name ln-custom-file-directory)
    (loce ln-custom-file-file-name))
