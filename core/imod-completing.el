...
(defun ln:smart-completions-sort (all)
  (pcase (ln:completion-metadata-get 'category)
    ('kill-ring all)
...
(defun ln/minibuffer-delete-char ()
  (interactive)
  (unless (and (eolp)
               (when (and (> (point) (minibuffer-prompt-end))
                          (eq 'file (ln:completion-metadata-get 'category)))
  ...
  (if (eolp)
      (let ((contents (minibuffer-contents)))
        ...
        (unless (string-match-p " URL" (minibuffer-prompt))
          ...
          (when (and (eq 'file (ln:completion-metadata-get 'category)) (not (string= contents "~/")))
            (insert "~/"))))
    (call-interactively #'kill-line)))

(defun ln:capf-functions-of-mode (mode)
...
(defmacro ln:exit-minibuffer-with-message (args &rest body)
...
(defmacro ln:with-completions-window (&rest body)
...
(cl-macrolet ((make-minibuffer-command (name &rest body)
                `(defun ,(intern (format "ln/minibuffer-%s" name)) ()
                   (interactive)
                   (ln:with-completions-window ,@body))))
  ...
  (cond ((get-buffer-window "*Completions*" 0)
        ...
        (vertico-mode
         (pcase type
           ...
           (:delete (setq vertico--candidates (ln:remove-nth vertico--index vertico--candidates))
                    (setq vertico--total (length vertico--candidates)))
           ((pred numberp) (vertico--goto vertico--index))))))

(defun ln:completion-metadata-get (what)
...
(setq minibuffer-completion-auto-choose t
      ...
      completions-sort #'ln:smart-completions-sort)

(x minibuffer
   :bind ( :map minibuffer-local-map
           ("C-d"      . ln/minibuffer-delete-char)
           ("C-k"      . ln/minibuffer-kill-line)
           ...
           ("C-v"      . ln/minibuffer-next-page)
           ("M-v"      . ln/minibuffer-previous-page)
           ("C-l"      . ln/minibuffer-recenter-top-bottom)
           ("M->"      . ln/minibuffer-end-of-buffer)
           ("M-<"      . ln/minibuffer-beginning-of-buffer)
...
(define-minor-mode ln/smart-completion-styles-mode nil :global t
  (if ln/smart-completion-styles-mode
      (defun:around completion-all-completions (fn &rest args)
        (if (or (memq (cadr args) '(ln:rg-completion-table)))
...
(ln/smart-completion-styles-mode 1)



(x corfu
   :ref "minad/corfu"
   :bind (:map corfu-map ([tab] . ln:first-yas-then-corfu))
   :init
...
(defvar ln:consult--source-buffer
  `( :name "Buffer"
     :narrow   ?b
     :category buffer
     :face     consult-buffer
...
(defvar ln:consult--source-erc-buffer
  `( :name "irc/matrix"
     :narrow   ?i
     :category erc
     :face     consult-buffer
...
(x consult
   ...
   (setopt consult-preview-key "C-o"
           consult-buffer-sources '(consult--source-hidden-buffer
                                    ln:consult--source-buffer
                                    ln:consult--source-erc-buffer
