...
(transient-define-prefix im/transient-dashbox () ; C-c d
  ...
  [[("f"  "Files"      (lambda () (interactive) (im:run-choose-command 'ztree-diff 'ztree-dir 'find-dired 'find-grep-dired)))
   ...
   [("p"  "Print"      im/transient-print)
    ("c"  "Agenda"     im/transient-agenda)]
   [("s"  "Setup"      im/transient-setup)
...
(transient-define-prefix im/transient-setup () ; C-c s
  [:hide (lambda () t)]
  [("f"   "setup font" (lambda () (interactive) (im:run-choose-command "Set font with: " 'f/font-default 'f/font-unicode 'f/font-of-face 'f/font-of-face-buffer-local)))
   ("t"   "setup theme" consult-theme)
   ("m"   "setup mode-line-height"   f/set-mode-line-height)
