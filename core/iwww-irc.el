...
(x erc
   :bind (:map erc-mode-map ("M-m" . ln:erc-go-to-begin))
   :config
   (setopt erc-server "irc.libera.chat"
           erc-nick "tzlm454"
           erc-fill-column 120
           erc-fill-static-center 14
           erc-fill-function 'ln:erc-fill-static--skip-system-message
           erc-insert-timestamp-function 'ln:erc-insert-timestamp-above
           ...
           erc-log-filter-function 'ln:erc-filter
...
(defun ln:erc-fill-static--skip-system-message ()
...
(defun ln:erc-toggle-hide-list ()
  (interactive)
  (setq erc-hide-list (if erc-hide-list nil erc-hide-list-default)))

(defun ln/erc-join-groups (&optional arg)
  (interactive)
  ...
  (let* ((candidates '("#emacs" "#commonlisp" "#c_lang_cn" "#linuxba"
         ...
         (groups (cond ((null arg)
                        (completing-read-multiple "Groups: " (ln:completion-table candidates)))
                       ((or (numberp arg)
...
(defun erc-cmd-JJ (&optional amount)
  (ln/erc-join-groups amount))



(transient-define-prefix ln/assist-erc-mode ()
  ...
  /nickname,list,kick,mode  /join,names,whois,quit  /msg,query,say,me..\n"
   ...
   [("-o" (lambda ()
            (if erc-hide-list
              ...
              "Sys Msg"))
     ln:erc-toggle-hide-list)
    ]
   [("o" "Occur.." erc-occur)
    ]
   [("s" "Save Buffer in Logs" erc-save-buffer-in-logs)
    ]
   [("m" "Join.." ln/erc-join-groups)
    ]
   ]
  (interactive)
  (if (eq major-mode 'erc-mode)
      (transient-setup 'ln/assist-erc-mode)
