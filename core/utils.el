...
(defmacro ln:compose (&rest functions)
...
(defmacro ln:with-message (action &rest body)
  "Usage:
(ln:with-message nil ...) to suppress all messages.
(ln:with-message sym ...) to dispatch messages to function sym.
(ln:with-message sexp ...) to dispatch messages to sexp, ie: lambda.
"
  (declare (indent 1))
  (if action
...
(defun ln:completion-table (collection &optional sort-fn category)
...
(defmacro ln:with-current-view-buffer (buffer &rest keywords-and-body)
...
(cl-defun ln:buffer-view (message &key buffer (focus t) (wc '((display-buffer-below-selected))) fontify post)
  "Popup a buffer to display message."
  (declare (indent 1))
  (let ((buf (or buffer (get-buffer-create (format "*view-%d*" (float-time))))))
    (ln:with-current-view-buffer buf
...
(defun ln:string-pad (item &optional needle)
...
(defun ln:join-as-regor-group (&rest items)
  "Join ITEMS as \\(aaa\\)\\|\\(bbb\\) style."
  (mapconcat (lambda (x) (format "\\(%s\\)" x)) items "\\|"))

(defmacro ln:string-interpolation (string)
...
(defun ln:plist-merge (&rest plists)
...
(defun ln:read-file-content (file &optional callback)
...
(cl-defmacro ln/walk-directory ((dir regexp &optional include-directories predicate follow-symlinks) &rest body)
...
(defun ln:thing-at-region-or-point (&optional thing)
...
(defun ln:in-comment-p (&optional pos)
...
(defun ln:make-buffer-buriable ()
  (set-buffer-modified-p nil)
  (page-break-lines-mode 1)
  (local-set-key "q" 'bury-buffer))

(defun ln:replace-all-in-buffer (list &optional pre-hook)
  "Replace all occurs in current buffer. eg:\n
\\=(ln:replace-all-in-buffer '((aaa . bbb) (ccc . ddd)) (lambda () (message xxx))) "
...
(defun ln:window-id (&optional window)
...
(defun ln:run-choose-command (&rest commands)
  (let ((prompt (if (stringp (car commands)) (pop commands) "Command: ")))
    (call-interactively (intern (completing-read prompt (ln:completion-table commands) nil t)))))

(defun ln:execute-command-matching (prompt include &optional exclude)
...
(defun ln/smart-yank ()
  (interactive)
  (if (use-region-p)
      (call-interactively #'ln/duplicate-lines)
    (call-interactively #'yank)))

(defun ln/smart-kill-ring-save ()
...
(defun ln/rectangle-number-lines ()
...
(defun ln/go-to-char (&optional backwardp)
...
(defun ln/match-paren (&optional ins)
  "Like `%' in vim."
  (interactive "P")
  (if ins (insert "%")
  ...
  (with-current-buffer
    ...
    (let ((i 0))
      ...
      (while (< i 32)
        ...
        (setq i (+ i 1)))
      (goto-char (point-min)))
    (ln:make-buffer-buriable)
    (local-set-key "Q" 'kill-this-buffer)
    (read-only-mode 1)))

(defun ln/pick-lines (&optional regexp)
...
(defun ln/duplicate-lines (&optional n)
...
(defun ln/count-lines-in-directory ()
  (interactive)
...
(defun ln/insert-date (&optional timep)
  (interactive "P")
  (insert (format-time-string (if timep "%F %T" "%F"))))

(defun ln/yank-current-directory ()
  (interactive)
...
(defun ln/yank-current-buffer-name ()
  (interactive)
...
(defun ln/yank-current-full-name (e)
  (interactive "e")
...
(defun ln/yank-current-dir-and-buffer-name (e)
  (interactive "e")
...
(defun ln/set-indent-width-or-offset ()
  (interactive)
...
(defun ln/describe-major-mode-keymap (&optional mode)
  (interactive)
...
(defvar ln-temp-log-buffer " look-the-temp-log")
...
(defmacro cw (&rest inputs)
  ...
  (let (messages buffer)
    ...
    `(with-current-buffer (get-buffer-create (or ,buffer ln-temp-log-buffer))
       (local-set-key "q" #'bury-buffer)
       ...
       (let ((line (propertize "⌁ " 'face 'font-lock-comment-face)))
         (cl-labels ((ladd (x) (setq line (concat line x)))
           ...
           ,@(cl-loop for _m in messages
                      ...
                      collect `(ladd (ln:string-interpolation ,norm)))
           (ladd "\n")
           (goto-char (point-max))
           (insert line)
...
(defun cw-1 () (interactive) (display-buffer (get-buffer-create ln-temp-log-buffer)))
