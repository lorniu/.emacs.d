...
(defun f/get (item &optional default)
  (let* ((faces-default (list :title "hi" :theme nil :font nil :font-u nil :font-mono '("Ubuntu Mono" "隶书")))
...
(defun f/default-font (&optional font height)
  "Set system default font."
  (interactive)
  (if (consp font) (setq height (cdr font) font (car font)))
  (let ((ft (f/find (unless (called-interactively-p 'any) font)))
        (fh (if (called-interactively-p 'any) (read-number "Font Height: " f/face-font-height) height))
        (args '(default nil)))
...
(defun f/unicode-font (&optional font height type)
  "Set the font of Unicode symbols, like 'han for Chinese."
  (interactive)
  (let* ((type (or type 'unicode))
         (ft (f/find (unless (called-interactively-p 'any) font)))
         (fh (if (called-interactively-p 'any) (read-number "Font Height: " f/face-font-height) height))
         (args))
    (if ft (setq args (append args (list :family ft))))
...
(defvar f/face-font-height 0)
(defvar f/face-font-family (f/get :font-mono)) ; Mono as default
(defvar f/face-font-extras '())

(defun f/face-font (&rest face-or-faces)
  ...
  (let* ((ft (f/find (unless (called-interactively-p 'any) f/face-font-family)))
         (fh (if (called-interactively-p 'any) (read-number "Font Height: " f/face-font-height) f/face-font-height))
         (args (list :family ft)))
    (if (and fh (> fh 0)) (setq args (append args (list :height fh))))
    (if f/face-font-extras (setq args (append args f/face-font-extras)))
...
(defun f/face-font-buffer-local (&rest face-or-faces)
