...
;;
;;   (ln/interactive-script "notes-publish-to-vps.sh")    ; rsync *.html to vps
;;   (url-retrieve "https://${vps}/sys/pull-and-reload")  ; execute `git pull` on vps
;;
;; Capture Content from Browser
;;
;;   1. Enable Org Protocol:
;;      - Linux (ln/org-protocol-enable-for-linux)
;;      - Windows (ls/emacs-generate-configs-for-windows)
;;
...
(defun ln/note-publish (&optional force)
  (interactive)
  (with-supress-recentf
   (require 'ox-publish)
   (let ((start (current-time))
         (ln-temp-log-buffer "*note-publish-log*"))
       ...
       (let ((default-directory org-directory)
         ...
         (unwind-protect
             (let ((gc-cons-threshold most-positive-fixnum))
               (run-hook-with-args 'note-publish-pre-hook ln-temp-log-buffer)
               (cw ">>> publish alist <<<\n{org-publish-project-alist}\n")
               (message "Publishing notes %s..." (propertize org-directory 'face 'font-lock-type-face))
               (ln:with-message (lambda (fmt &rest args) (cw (apply #'format (or fmt "") args)))
               ...
               (run-hook-with-args 'note-publish-post-hook ln-temp-log-buffer))
           (kill-buffer)))))))

(defun ln/note-publish-interactively (&optional force)
   ...
   (let ((org-directory (read-directory-name "Publish From: ")))
     (with-current-buffer (find-file-read-only org-directory)
       (let ((org-publish-project-alist (or org-publish-project-alist
         ...
         (ln/note-publish force))))))

(defun ln/note-publish-reset-directories ()
  (interactive)
