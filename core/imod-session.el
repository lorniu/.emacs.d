...
(x desktop
   ...
   ;; So can't add `ln:fullscreen-alist' here
   (add-to-list 'desktop-globals-to-save 'ln:views)
...
(x savehist
   ...
   (setopt savehist-file (locc "history")
           savehist-ignored-variables '(ln:pages
                                        ln:rg-search-history
                                        consult--theme-history
                                        consult-org--history
                                        query-replace-history)))

(x recentf
   :init (ln:with-message nil (recentf-mode 1))
   :config
...
(defun ln/recentf-clear (&optional regexp)
...
(defun ln:desktop-read-dir (&optional write)
...
(defun ln:desktop-save-to (dir)
  (desktop-save dir)
  (message (concat "Saved to " (propertize dir 'face 'font-lock-string-face) ", done!")))

(transient-define-prefix ln/transient-desktop ()
  [:hide
   (lambda () t)
   ("f  "  "1"  (lambda () (interactive) (desktop-read (ln:desktop-read-dir))))
   ("C-f"  "2"  (lambda () (interactive) (desktop-read (ln:desktop-read-dir))))
   ("C-w"  "3"  (lambda () (interactive) (ln:desktop-save-to (ln:desktop-read-dir t))))
   ...
   ("C-d"  "6"  (lambda () (interactive) (ln:desktop-save-to desktop-save-default-dir)))
   ("k  "  "7"  desktop-clear)]
  [[("w" (lambda () (!tdesc "f w" "Find/Save..."))      (lambda () (interactive) (ln:desktop-save-to (ln:desktop-read-dir t))) :format "%d")]
   [("c" "Change..."                                    (lambda () (interactive) (desktop-change-dir (ln:desktop-read-dir))))]
   [("d" (lambda () (!tdesc "o d" "Load/Save Default")) (lambda () (interactive) (ln:desktop-save-to desktop-save-default-dir)) :format "%d")]
