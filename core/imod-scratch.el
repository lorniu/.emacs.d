...
(defun scratch-restore ()
  ...
  (let* ((files (or (im:scratch-saves)
         ...
         (cands (mapcar (lambda (f)
                        ...
                        files))
         (annfn (lambda (name)
                  (when-let* ((file (cdr (assoc name cands)))
                              (attrs (file-attributes file))
                              (size (file-attribute-size attrs))
                              (len (min 90 (max 40 (- (frame-width) 25))))
                              (prev (let* ((c (with-temp-buffer (insert-file-contents file) (buffer-string)))
                                           (s (string-trim (replace-regexp-in-string "[ \n]+" " " c)))
                                           (tail (substring s (max 0 (- (length s) len)))))
                                      (string-truncate-left tail (- len 20)))))
                    (format "%s  %12s %5s  %s"
