...
(defvar ln/scratch-location (locc))
(defvar ln/scratch-file-prefix "scratch_")
(defvar ln/scratch-keeps 9)

(defun ln/scratch-saves ()
  (sort (directory-files ln/scratch-location t (concat "^" ln/scratch-file-prefix "[0-9]\\{8\\}\\.*[0-9]\\{6\\}$")) #'string>))
  ...
  (condition-case err
      (let* ((filename (format "%s%s%s"
        ...
        (if (or (< ln/scratch-keeps 1)
          ...
          (when (>= (length saves) ln/scratch-keeps)
            (cl-loop for f in saves for i from 1
                     if (> i ln/scratch-keeps) do (delete-file f)))
...
(defun scratch-buffer-restore ()
  ...
  (interactive)
  (let* ((files (or (ln/scratch-saves) (user-error "No saved scratches found")))
    ...
    (cond ((string-empty-p file)
           (with-current-buffer (find-file ln/scratch-location)
...
(transient-define-prefix ln/assist-*scratch* ()
  [[("m"
