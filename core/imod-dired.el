...
(xzz dired
  ...
  :bind ( :map dired-mode-map
          ...
          ( "SPC" . im/dired-preview-file )
          :map wdired-mode-map
          ( "C-c SPC" . im/dired-preview-file ))
  :config
...
(defun im/dired-preview-file (&optional stay)
  "Preview file at point like macOS.
With prefix arg or in wdired-mode, stay on current file."
  (interactive "P")
  (let ((file (dired-get-filename nil t))
        (stay (or stay (eq major-mode 'wdired-mode))))
    (cond
     ;; No file -> jump to first valid file
     ((null file)
      (goto-char (point-min))
      (while (and (not (eobp))
                  (let ((f (dired-get-filename nil t)))
                    (or (null f) (member (file-name-nondirectory f) '("." "..")))))
        (forward-line 1))
      (when (dired-get-filename nil t) (dired-move-to-filename)))

     ;; Directory -> enter
     ((file-directory-p file)
      (dired-find-file))

     ;; File -> preview or advance
     (t
      (let* ((buf (get-file-buffer file))
             (win (and buf (get-buffer-window buf)))
             (previewed (and win (not (eq win (selected-window)))))
             (ext (downcase (or (file-name-extension file) ""))))

        (if (and previewed (not stay))
            ;; Already previewed and not staying -> move to next or close
            (let ((pos (point)))
              (forward-line 1)
              (if (and (dired-get-filename nil t)
                       (not (eobp)))
                  (progn
                    (dired-move-to-filename)
                    (let ((next-file (dired-get-filename nil t)))
                      (when (and next-file (not (file-directory-p next-file)))
                        (delete-window win)
                        (dired-display-file))))
                (goto-char pos)
          ...
          ;; Not previewed or staying -> open based on type
          (cond
           ((member ext '("zip" "tar" "gz" "tgz" "rar" "7z" "bz2" "xz"))
            (dired-find-file))

           ((member ext '("mp4" "mkv" "avi" "mp3" "wav" "flac" "mov" "webm"))
            (condition-case err
                (start-process "dired-av" nil
                              (if (eq system-type 'darwin) "open" "xdg-open")
                              file)
              (error (message "Failed to open %s: %s" file (error-message-string err)))))

           (t (dired-display-file)))))))))
