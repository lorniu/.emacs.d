...
(x dired
   ...
   :bind ( :map dired-mode-map
	       ...
	       ( "^" . ln/dired-up-directory-follow-symlink )
	       ( "e" . wdired-change-to-wdired-mode )
           ( "g" . (lambda () (interactive) (ln/dired-sort)) )
           ( "s" . ln/dired-sort )
	       ( "z" . ln/dired-du-size )
	       ( "Y" . ln/dired-rsync )
	       ( "," . ln/dired--toggle-collapse ) )
   :config
   ...
   (defvar ln:ls-lisp-xid-shorten-threshold (if IS-WIN 8)
   ...
   (defun:before ls-lisp-format//shorten (_fn file-attr _fs _sw _ti)
	 (when ln:ls-lisp-xid-shorten-threshold
	   (cl-labels ((norm (file n &optional
					           (threshold ln:ls-lisp-xid-shorten-threshold)
					           (len (- ln:ls-lisp-xid-shorten-threshold 3)))
				     (let ((item (nth n file)))
...
(x dired-collapse/e
   :init
   (defvar ln:dired-collapse-enable t)
   (defun:hook dired-mode-hook/collapse ()
     (dired-collapse-mode (if ln:dired-collapse-enable 1 -1)))
   (defun ln/dired--toggle-collapse (&optional arg)
     "Toggle collapse, with `C-u' then futrue will effact future dired buffer."
     (interactive "P")
     (let (current-prefix-arg)
       (call-interactively 'dired-collapse-mode))
     (when arg
       (setq ln:dired-collapse-enable dired-collapse-mode)
       (message "Global dired-collapse mode: %s" ln:dired-collapse-enable))))

(defun ln/dired-du-size ()
...
(defun ln/dired-up-directory-follow-symlink (&optional arg)
  (interactive "P")
...
(defvar ln:dired-sort-list '((:name "" -1) (:date "-t" -1) (:size "-S" -1) (:extension "-X" 1)))

(defvar-local ln:dired-sort-current nil)

(defun ln/dired-sort (&optional key order)
  "Sort dired by the given criteria.
When 'current-prefix-arg' non-nil then fall back to builtin dired-sort."
  (interactive)
  (if (called-interactively-p 'any)
      (call-interactively (if current-prefix-arg #'dired-sort-toggle-or-edit #'ln/transient-dired-sort))
    ...
    (if (and (null key) (null order)) ; revert
        (setq dired-actual-switches dired-listing-switches
              ...
              ln:dired-sort-current nil)
      (unless key (setq key :name)) ; default sort by name
      (unless order ; reverse or default
        (setq order (if (equal key (car-safe ln:dired-sort-current))
                        (* -1 (cdr-safe ln:dired-sort-current))
                      (caddr (assoc key ln:dired-sort-list)))))
      (let ((switches (format "%s %s %s"
                              dired-listing-switches
                              (cadr (assoc key ln:dired-sort-list))
                              (if (cl-plusp order) "" "-r"))))
        ...
        (setq mode-name (format "Dired by %s %s"
                                ...
                                (if (cl-plusp order) "⇡" "⇣")))
        (setq ln:dired-sort-current (cons key order))))
    (force-mode-line-update)
    (revert-buffer)))

(transient-define-prefix ln/transient-dired-sort ()
  :transient-non-suffix #'transient--do-exit
  [:hide
   (lambda () t)
   ("N"  "1"          (lambda () (interactive) (ln/dired-sort :name 1)))
   ("D"  "2"          (lambda () (interactive) (ln/dired-sort :date 1)))
   ("S"  "3"          (lambda () (interactive) (ln/dired-sort :size 1)))
   ("E"  "4"          (lambda () (interactive) (ln/dired-sort :extension 1)))
   ("g"  "5"          (lambda () (interactive) (ln/dired-sort)))]
  [[("n" "Name"       (lambda () (interactive) (ln/dired-sort :name))      :transient t)]
   [("d" "Date"       (lambda () (interactive) (ln/dired-sort :date))      :transient t)]
   [("s" "Size"       (lambda () (interactive) (ln/dired-sort :size))      :transient t)]
   [("e" "Extension"  (lambda () (interactive) (ln/dired-sort :extension)) :transient t)]]
  (interactive)
  (if (eq major-mode 'dired-mode)
      (transient-setup 'ln/transient-dired-sort)
    (user-error "Should be used in Dired")))


;;; Interface (Helper)

(transient-define-prefix ln/assist-dired-mode ()
  :transient-non-suffix 'transient--do-exit
  [["Dired"
  ...
  (if (eq major-mode 'dired-mode)
      (progn
        (transient-setup 'ln/assist-dired-mode)
