...
(defvar ln-eshell-extra-aliases nil)
...
(x eshell
   ...
   (defun:after eshell-read-aliases-list//extra (&rest _)
     (when (and ln-eshell-extra-aliases (not (get 'ln-eshell-extra-aliases 'done)))
       (setopt eshell-command-aliases-list (append ln-eshell-extra-aliases eshell-command-aliases-list))
       (put 'ln-eshell-extra-aliases 'done t)))
   ...
   (defun:hook eshell-mode-hook/capf ()
     (add-hook 'completion-at-point-functions 'ln:eshell-cd-competion-at-point nil 'local)))
         ...
         (ret (minibuffer-with-setup-hook
                  (lambda ()
                    ...
                    (local-set-key (kbd "C-k")
                                   (lambda ()
                                     (interactive)
                                     (when-let* ((idx (get-pos-property 0 'ring-idx (ln:completion-compat :current))))
                                       (when (with-current-buffer buf (ring-remove eshell-history-ring idx))
                                         (ln:completion-compat :delete))))))
                (completing-read "Command: " (ln:completion-table hists)
                                 nil nil nil nil (if (cl-plusp (length input)) input (car hists))))))
    (delete-region start-pos end-pos)
    (insert ret)))

(defun ln:eshell-cd-competion-at-point ()
  (when (eobp)
      ...
      (when (string-match-p "^cd[^ ]*$" (buffer-substring beg end))
        (let* ((sortfn (lambda (x y)
               ...
               (others (list (loce "") (locc) org-directory agenda-directory
                             ln-workdir ln-srcdir ln-downdir (if IS-WIN (getenv "USERPROFILE"))))
...
(when IS-LINUX
  ...
  (cl-defmethod ln:local-assist ((_ (eql 'eat-mode)))
    (if eat--semi-char-mode (eat-char-mode) (eat-semi-char-mode)))

  (cl-defmethod ln:local-assist ((_ (eql 'eshell-mode)))
...
(defun ln/toggle-eat-buffer ()
  "Toggle show the *eat* buffer."
  (interactive)
  (ln/hide-or-show-buffer "*eat*"
...
(defun ln/toggle-eshell-buffer ()
  "Toggle show the *EShell* buffer."
  (interactive)
  (ln/hide-or-show-buffer "*eshell*"
    (let ((display-buffer-alist '(("*" (display-buffer-reuse-window %display-buffer-in-direction-or-at-bottom) (direction . right)))))
      (call-interactively 'eshell))))

(defun ln/popup-xshell ()
  (interactive)
  (let ((type (completing-read "Open: "
    ...
    (pcase (intern type)
      ...
      ('system-default (ln/popup-system-terminal)))))

(defun ln/popup-system-terminal ()
  "Open system terminal."
  (interactive)
  (if ln-system-terminal
      (let ((d (or default-directory "~/")))
        (start-process-shell-command (format "sys-terminal-%s" d) nil (format ln-system-terminal d))
        (message "OK."))
    (user-error "Please config `ln-system-terminal' first")))
