...
(xzz eshell
  ...
  (defun:after eshell-read-aliases-list//extra (&rest _)
    (when (and im.eshell-extra-aliases (not (get 'im.eshell-extra-aliases 'done)))
      (setq eshell-command-aliases-list (append im.eshell-extra-aliases eshell-command-aliases-list))
...
(when IS-LINUX
  ...
  (cl-defmethod im:local-assist ((_ (eql 'eat-mode)))
    (if eat--semi-char-mode (eat-line-mode) (eat-semi-char-mode)))

  (cl-defmethod im:local-assist ((_ (eql 'eshell-mode)))
    (if eat--eshell-char-mode (eat-eshell-semi-char-mode) (eat-eshell-char-mode))))
