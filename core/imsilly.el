...
(defun ln/interactive-script (&optional script buffer)
...
(defun ls/emacs-recompile-els (&optional force)
  "Recompile .el files."
  (interactive "P")
  (let* ((def (loce "extra/"))
...
(defun ls/emacs-generate-configs-for-windows ()
...
(defun ls/emacs-current-build-configure ()
  (interactive)
...
(defun ls:convert-encodings-in-directory ()
  "Emacs style batch mode / change coding under the dir to UTF8."
  (ln/walk-directory ("/var/www/" ".*\\.rb")
...
(cl-defun ls:dump-pg-to-mysql (table-name dest-file &optional (dbname "imdata"))
...
(defun ls/generate-ssh-config-file ()
...
(defun ls/generate-nat-traverse-cmd (name vps inner)
...
(defun ls/windows-install-scoop-or-chocolatey ()
  (interactive)
...
(defun ls/youtube-dl-url (&optional url)
