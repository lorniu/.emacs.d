...
;;                :alpha 88
;;                :hook (lambda() ...))
;;           (bbb* :...))) ; [.*~] as suffix is used as default
;;
;;   (setq ic/faces '(:height 200 :font "xxx")) ; for simple
...
(defun f/get (item &optional default)
  (when (and ic/faces (atom (car ic/faces)))
    (setq ic/faces (list (cons 'default ic/faces))))
  (let ((faces (cdr (or (and (string-match-p "^[0-9]+$" (concat (getenv-internal "face")))
                             (nth (string-to-number (getenv-internal "face")) ic/faces))
    ...
    (cond
     ...
     ((not (keywordp item)) (cdr (assoc item ic/faces)))
...
;; Frame Commands, with preset configs support

(defun f/set (param value)
  "Set frame parameters interactively."
  (interactive (let* ((nps '(alpha alpha-background line-spacing font left-fringe right-fringe))
                      ...
                      (param (intern (completing-read "Set Frame: "
                                                      (append '(*preset* mode-line-height) (append nps cps))
                                                      nil t nil 'f/set--history)))
                      ...
                      (value (cond
                              ...
                              ((eq param '*preset*)
                               (intern (completing-read "Preset: " ic/faces #'cdr t)))
    ...
    (if (or interactivep (and (null regexp) (null all)))
        (let* ((completion-ignore-case t)
               ...
               (fonts (mapcar (lambda (f)
                                (cons f (if-let (fi (font-info f)) (file-name-directory (aref fi 12)))))
  ...
  (interactive (let* ((face (or f/current--face
                      ...
                      (fontsets (let ((ls (fontset-list))
                                  ...
                                  (if (ignore-errors (fontset-name-p cr))
                                    ...
                                    ls)))
                      (fontset (completing-read
                                "Fontset (for Non-ASCII): "
                                (lambda (input pred action)
                                  (if (eq action 'metadata)
                                      `(metadata (display-sort-function . ,#'identity))
                                    (complete-with-action action fontsets input pred))))))
...
(setq frame-title-format (f/get :title `((:eval (cond ((equal (elt (buffer-name) 0) ? ) "Nix") (t "%b"))))))
...
(when IS-NG
  ...
  (unless (f/get :theme) (setf (plist-get (cdar ic/faces) :theme) 'origin))
  (xterm-mouse-mode)
