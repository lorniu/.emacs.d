...
(defun ln/httpd-here (&optional arg)
  (interactive "P")
...
(defun ln/wifi-nmcli (&optional initial-input)
...
(defun ln/network-ipinfo-query (ip)
...
(defun ln/network-dns-query (&optional host)
  (interactive (list (read-string "Host name: " nil 'ln:network-dns-history)))
...
(defun ln/network-local-ip-address (&optional dev)
...
(defun ln/start-localhost ()
  "Run a local static server interactively."
  (interactive)
  (let* ((getval (lambda (item)
                   (or (assoc item ln-local-servers) (intern item))))
         (dispfn (lambda (item)
                                            ...
                                            (lambda (input pred action)
                                              (if (eq action 'metadata)
                                                ...
                                                (complete-with-action action
                                                                      (cl-loop for s in ln-local-servers
...
(defun ln/network-interface ()
  (interactive)
  (ln:run-choose-command
   ...
   'ln/proxy 'ln/network-ipinfo-query 'ln/network-dns-query 'ln/network-local-ip-address 'ln/start-localhost))
