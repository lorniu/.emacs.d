...
(defun im/start-webserver ()
  "Run a local static server interactively."
  (interactive)
  (let* ((getval (lambda (item)
                   (or (assoc item ic/webservers) (intern item))))
         (dispfn (lambda (item)
                                            ...
                                            (lambda (input pred action)
                                              (if (eq action 'metadata)
                                                ...
                                                (complete-with-action action
                                                                      (cl-loop for s in ic/webservers
...
(defun im/network-interface ()
  ...
  (im:run-choose-command
   ...
   'im/proxy 'im/network-ipinfo-query 'im/network-dns-query 'im/network-local-ip-address 'im/start-webserver))
