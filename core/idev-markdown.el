...
(x markdown-mode
   ...
   :bind (:map markdown-mode-map ("C-c l" . ln/markdown-hide-levels))
   :config
...
(transient-define-prefix ln/assist-markdown-mode ()
  [:hide
   (lambda () t)
  ...
  (if (memq major-mode '(markdown-mode gfm-mode))
      (transient-setup 'ln/assist-markdown-mode)
    (user-error "You should invoke this in markdown-mode or gfm-mode")))

(defalias 'ln/assist-gfm-mode 'ln/assist-markdown-mode)



(defun ln/markdown-hide-levels ()
  "Hide to Lv-2."
  (interactive)
  (save-excursion
    (goto-char (point-min))
